            ...
            data.map((item) => {
                return (
                    <React.Fragment key={item.id}>
                        {!!item?.subMenu ? (
                            <li className='relative cursor-pointer flex items-center gap-2' onMouseEnter={()=> setShowCatMenu(true)} onMouseLeave={()=> setShowCatMenu(false)}>
                                ...
                                showCatMenu && (
                                    <ul className='bg-white absolute top-6 left-0 min-w-[250px] py-1 px-1 text-black shadow-lg'>
                                        {categories.map(({id, attributes:category}) => {
                                            return(
                                                <Link href={`/category/${category?.slug}`} key={id} onClick={()=> setShowCatMenu(false)}>
